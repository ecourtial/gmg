...
class CopyRepository(AbstractRepository):
    ...
    def get_select_request_start(self):
        ...
        request += 'FROM '
        request += f"     (SELECT COUNT(*) AS transactionCount, {Copy.table_name}.{Copy.primary_key} AS copy_id "
        ...
        request += f"WHERE {Copy.table_name}.version_id = {Version.table_name}.{Version.primary_key} "
