...
class GameRepository(AbstractRepository):
    def get_by_id(self, platform_id):
        ...
        request = "SELECT id, title FROM games "
    ...
    def insert(self, game):
        """Insert a new game"""
        request = "INSERT INTO games (title) VALUES (%s)"
        self.write(request, (game.get_title(),))

        return self.get_by_title(game.get_title())

    def update(self, game):
        """Update a game"""
        request = "UPDATE games SET title = %s WHERE id = %s;"
        self.write(request, (game.get_title(), game.get_id()))
