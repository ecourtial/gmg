...
class TransactionRepository(AbstractRepository):
    ...
    def get_select_request_start(self):
        ...
        request += f"{Platform.table_name}.name AS platformName, {Version.table_name}.{Version.primary_key} as version_id "
        request += f"FROM {Transaction.table_name}, {Copy.table_name}, {Version.table_name}, {Game.table_name}, {Platform.table_name} "
