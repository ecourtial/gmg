...
class TransactionService(AbstractService):
    ...
    def get_for_create(self) -> Transaction:
        return super().validate_payload_for_creation_and_hydrate(Transaction)

    def get_for_update(self, transaction_id: int) -> Transaction:
    ...
    def insert(self, object: Transaction) -> Transaction:
    ...
    def update(self, object: Transaction) -> Transaction:
        version = self.get_version(object)
        copy = self.get_copy(object)
        self.check_no_duplicate_type_consecutive(copy, object)
        self.check_version_copy_consistency(version, copy)
    ...
    def get_version(self, transaction: Transaction) -> Version:
    ...
    def get_copy(self, transaction: Transaction) -> Copy | None:
    ...
    def check_version_copy_consistency(self, version: Version, copy: Copy | None) -> None:
    ...
    def update_copy_status(self, transaction: Transaction, copy: Copy | None) -> None:
        if copy is not None:
    ...
    def update_copy_in_db(self, transaction: Transaction, copy: Copy | None) -> None:
        if copy is not None:
    ...
    def is_sold_transaction(self, transaction: Transaction) -> bool:
        if transaction.get_type() == 'Sold':
            return True

        return False

    def update_copy_id(self, transaction: Transaction) -> None:
        if self.is_sold_transaction(transaction):
            transaction.set_copy_id(None)

    def check_no_duplicate_type_consecutive(self, copy: Copy | None, transaction: Transaction) -> None:
        if copy is not None:
