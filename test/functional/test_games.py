...
class TestPlatforms(AbstractTests):
    def test_commons(self):
        super().check_all_routes_error_bad_user_token('games')
        super().check_all_routes_error_missing_user_token('games')

    def test_get_game(self):
        # Does not exist
        resp = self.api_call('get', 'games/666', {}, True)

        self.assertEqual(404, resp.status_code)
        self.assertEqual({'message': "The resource of type 'game' with id #666 has not been found.", 'code': 1}, resp.json())

        # Exist
        resp = self.api_call('get', 'games/1', {}, True)
    ...
    def test_create_incomplete_payload(self):
        resp = self.api_call('post', 'games', {}, True)
    ...
    def test_create_duplicate_title(self):
        resp = self.api_call('post', 'games', {'title': 'Fifa 97', 'finished': False}, True)
    ...
    def test_create_update_delete_success(self):
        ...
        resp = self.api_call('get', 'games/' + game_id, None, True)
        payload['id'] = 381
        ...
        resp = self.api_call('patch', 'games/' + game_id, {'title': new_title}, True)

        self.assertEqual(200, resp.status_code)
        self.assertEqual(new_title, resp.json()["title"]) 

        # Delete
        resp = self.api_call('delete', 'games/' + game_id, {}, True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_update_duplicate_title(self):
        resp = self.api_call('patch', 'games/4', {'title': 'Fifa 97'}, True)
    ...
    def test_delete_fails_because_game_has_versions(self):
        resp = self.api_call('delete', 'games/1', {}, True)
