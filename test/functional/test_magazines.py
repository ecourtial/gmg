...
class TestMagazines(AbstractTests):
    def test_commons(self):
        super().check_all_routes_error_bad_user_token('magazines')
        super().check_all_routes_error_missing_user_token('magazines')

    def test_get_magazine(self):
        # Does not exist
        resp = self.api_call('get', 'magazines/666', {}, True)

        self.assertEqual(404, resp.status_code)
        self.assertEqual({'message': "The resource of type 'magazine' with id #666 has not been found.", 'code': 1}, resp.json())

        # Exists
        resp = self.api_call('get', 'magazines/1', {}, True)
    ...
    def test_create_incomplete_payload(self):
        resp = self.api_call('post', 'magazines', {}, True)
    ...
    def test_create_duplicate_title(self):
        resp = self.api_call('post', 'magazines', {'title': 'Gen4'}, True)
    ...
    def test_create_update_delete_success(self):
        ...
        resp = self.api_call('get', 'magazines/' + magazine_id, None, True)
        payload['id'] = 3
        ...
        resp = self.api_call('patch', 'magazines/' + magazine_id, {'title': new_title}, True)

        self.assertEqual(200, resp.status_code)
        self.assertEqual(new_title, resp.json()['title'])

        # Delete
        resp = self.api_call('delete', 'magazines/' + magazine_id, {}, True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_update_not_found(self):
        resp = self.api_call('patch', 'magazines/666', {'title': 'Whatever'}, True)

        self.assertEqual(404, resp.status_code)
        self.assertEqual({'message': "The resource of type 'magazine' with id #666 has not been found.", 'code': 1}, resp.json())

    def test_update_duplicate_title(self):
        resp = self.api_call('patch', 'magazines/2', {'title': 'Gen4'}, True)

        self.assertEqual(400, resp.status_code)
        self.assertEqual({'message': "The resource of type 'magazine' with title 'Gen4' already exists.", 'code': 8}, resp.json())

    def test_delete_fails_because_magazine_has_issues(self):
        resp = self.api_call('delete', 'magazines/1', {}, True)
