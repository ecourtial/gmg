...
class TestGameVersionMagazineMentions(AbstractTests):
    def test_commons(self):
        super().check_all_routes_error_bad_user_token('game-version-magazine-mentions')
        super().check_all_routes_error_missing_user_token('game-version-magazine-mentions')

    def test_get_mention(self):
        # Does not exist
        resp = self.api_call('get', 'game-version-magazine-mentions/666', {}, True)
        ...
        # Exists
        resp = self.api_call('get', 'game-version-magazine-mentions/1', {}, True)
    ...
    def test_create_incomplete_payload(self):
        resp = self.api_call('post', 'game-version-magazine-mentions', {}, True)

        self.assertEqual(400, resp.status_code)
        self.assertEqual({'message': 'The following field is missing: magazineIssueId.', 'code': 6}, resp.json())

    def test_create_unsupported_type(self):
        resp = self.api_call('post', 'game-version-magazine-mentions', {'magazineIssueId': 1, 'gameVersionId': 1, 'type': 'Review'}, True)

        self.assertEqual(400, resp.status_code)

    def test_create_magazine_issue_not_found(self):
        resp = self.api_call('post', 'game-version-magazine-mentions', {'magazineIssueId': 666, 'gameVersionId': 1, 'type': 'Test', 'pageNumber': 49}, True)
    ...
    def test_create_game_version_not_found(self):
        resp = self.api_call('post', 'game-version-magazine-mentions', {'magazineIssueId': 1, 'gameVersionId': 666, 'type': 'Test', 'pageNumber': 49}, True)
    ...
    def test_create_update_delete_success(self):
        ...
        resp = self.api_call('get', 'game-version-magazine-mentions/' + mention_id, None, True)
        payload['id'] = 4
        self.assertEqual(payload, resp.json())

        # Patch
        resp = self.api_call('patch', 'game-version-magazine-mentions/' + mention_id, {'notes': 'Updated.'}, True)

        self.assertEqual(200, resp.status_code)
        self.assertEqual('Updated.', resp.json()['notes'])

        # Delete
        resp = self.api_call('delete', 'game-version-magazine-mentions/' + mention_id, {}, True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_update_not_found(self):
        resp = self.api_call('patch', 'game-version-magazine-mentions/666', {'notes': 'Whatever'}, True)
