...
class TestMagazineIssues(AbstractTests):
    def test_commons(self):
        super().check_all_routes_error_bad_user_token('magazine-issues')
        super().check_all_routes_error_missing_user_token('magazine-issues')

    def test_get_magazine_issue(self):
        # Does not exist
        resp = self.api_call('get', 'magazine-issues/666', {}, True)

        self.assertEqual(404, resp.status_code)
        self.assertEqual({'message': "The resource of type 'magazine_issue' with id #666 has not been found.", 'code': 1}, resp.json())

        # Exists
        resp = self.api_call('get', 'magazine-issues/1', {}, True)
    ...
    def test_create_incomplete_payload(self):
        resp = self.api_call('post', 'magazine-issues', {}, True)
    ...
    def test_create_magazine_not_found(self):
        resp = self.api_call('post', 'magazine-issues', {'magazineId': 666, 'issueNumber': 1, 'year': 2000, 'month': 1}, True)
    ...
    def test_create_duplicate_issue_number(self):
        resp = self.api_call('post', 'magazine-issues', {'magazineId': 1, 'issueNumber': 1, 'year': 1997, 'month': 8}, True)
    ...
    def test_create_update_delete_success(self):
        ...
        resp = self.api_call('get', 'magazine-issues/' + issue_id, None, True)
        payload['id'] = 4
        self.assertEqual(payload, resp.json())

        # Patch
        resp = self.api_call('patch', 'magazine-issues/' + issue_id, {'notes': 'Mis à jour.'}, True)

        self.assertEqual(200, resp.status_code)
        self.assertEqual('Mis à jour.', resp.json()['notes'])

        # Delete
        resp = self.api_call('delete', 'magazine-issues/' + issue_id, {}, True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_update_not_found(self):
        resp = self.api_call('patch', 'magazine-issues/666', {'notes': 'Whatever'}, True)

        self.assertEqual(404, resp.status_code)
        self.assertEqual({'message': "The resource of type 'magazine_issue' with id #666 has not been found.", 'code': 1}, resp.json())

    def test_update_duplicate_issue_number(self):
        resp = self.api_call('patch', 'magazine-issues/3', {'issueNumber': 1}, True)

        self.assertEqual(400, resp.status_code)
        self.assertEqual({'message': "The resource of type 'magazine_issue' with issue_number '1' already exists.", 'code': 8}, resp.json())

    def test_delete_fails_because_issue_has_copies(self):
        resp = self.api_call('delete', 'magazine-issues/1', {}, True)
    ...
    def test_delete_fails_because_issue_has_mentions(self):
        resp = self.api_call('delete', 'magazine-issues/2', {}, True)
        ...
        resp = self.api_call('get', 'magazine-issues/2', {}, True)
        self.assertEqual(200, resp.status_code)
