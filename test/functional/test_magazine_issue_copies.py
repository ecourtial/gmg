...
class TestMagazineIssueCopies(AbstractTests):
    def test_commons(self):
        super().check_all_routes_error_bad_user_token('magazine-issue-copies')
        super().check_all_routes_error_missing_user_token('magazine-issue-copies')

    def test_get_magazine_issue_copy(self):
        # Does not exist
        resp = self.api_call('get', 'magazine-issue-copies/666', {}, True)

        self.assertEqual(404, resp.status_code)
        self.assertEqual({'message': "The resource of type 'magazine_issue_copy' with id #666 has not been found.", 'code': 1}, resp.json())

        # Exists
        resp = self.api_call('get', 'magazine-issue-copies/1', {}, True)
    ...
    def test_create_incomplete_payload(self):
        resp = self.api_call('post', 'magazine-issue-copies', {}, True)

        self.assertEqual(400, resp.status_code)
        self.assertEqual({'message': 'The following field is missing: magazineIssueId.', 'code': 6}, resp.json())

    def test_create_unsupported_type(self):
        resp = self.api_call('post', 'magazine-issue-copies', {'magazineIssueId': 1, 'type': 'Vinyl'}, True)

        self.assertEqual(400, resp.status_code)

    def test_create_magazine_issue_not_found(self):
        resp = self.api_call('post', 'magazine-issue-copies', {'magazineIssueId': 666, 'type': 'Printed-Original'}, True)
    ...
    def test_create_update_delete_success(self):
        ...
        resp = self.api_call('get', 'magazine-issue-copies/' + copy_id, None, True)
        payload['id'] = 3
        self.assertEqual(payload, resp.json())

        # Patch
        resp = self.api_call('patch', 'magazine-issue-copies/' + copy_id, {'notes': 'Updated note.'}, True)

        self.assertEqual(200, resp.status_code)
        self.assertEqual('Updated note.', resp.json()['notes'])

        # Delete
        resp = self.api_call('delete', 'magazine-issue-copies/' + copy_id, {}, True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_update_not_found(self):
        resp = self.api_call('patch', 'magazine-issue-copies/666', {'notes': 'Whatever'}, True)
